...
class FeedingChart extends StatefulWidget {
  final String petId;
  const FeedingChart({super.key, required this.petId});
...
class _FeedingChartState extends State<FeedingChart> {
  List<double> feedingValues = [0, 0, 0, 0, 0, 0, 0];
  bool isLoading = true;
  ...
  Future<void> fetchFeedingHistory() async {
    try {
      ...
      log(
        'Fetching feeding history for week: ${monday.toString()} to ${sunday.toString()}',
      );

      // Initialize counts for each day of the week
      List<int> dailyCounts = List.filled(7, 0);

      // Fetch feeding history for the current week
      final snapshot =
          await FirebaseFirestore.instance
              ...
              .where(
                'completedAt',
                isLessThanOrEqualTo: Timestamp.fromDate(sunday),
              )
              .get();

      log('Found ${snapshot.docs.length} feeding records');

      // Count feedings for each day
      for (var doc in snapshot.docs) {
        ...
        log('Feeding on ${completedAt.toString()}, day index: $dayIndex');
      ...
      setState(() {
        feedingValues = dailyCounts.map((count) => count.toDouble()).toList();
        isLoading = false;
      });
    } catch (e) {
      log('Error fetching feeding history: $e');
      setState(() {
        isLoading = false;
      });
    }
  ...
  List<String> getCurrentWeekDayNames() {
    return getCurrentWeekDates()
        .map((date) => DateFormat('EEE').format(date))
        .toList();
  }
          ...
          AspectRatio(
            ...
            child: BarChart(
              BarChartData(
                ...
                maxY:
                    feedingValues.isEmpty
                        ? 5
                        : feedingValues.reduce((a, b) => a > b ? a : b) + 1,
                minY: 0,
